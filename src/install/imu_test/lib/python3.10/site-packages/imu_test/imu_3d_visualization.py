...
class IMU3DVisualization(Node):
    ...
    def imu_callback(self, msg):
        ...
        # Create a Point message for the estimated position in the imu_link frame
        imu_position = Point()
        imu_position.x = self.filtered_pos_x
        imu_position.y = self.filtered_pos_y
        imu_position.z = self.filtered_pos_z

        # Transform the estimated position from imu_link frame to base_link frame
        base_position = tf2_geometry_msgs.do_transform_point(imu_position, transform)
